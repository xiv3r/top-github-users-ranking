...
The `public contributions` and `private contributions` by users in Brazil on `2025/4/11 1:48 PM UTC`. This list contains users from `Brazil` and cities `Rio-de-janeiro` `São-paulo` `Manaus` `Brasilia` `Fortaleza` `Belém` `Goiânia` `Belo-horizonte` `Curitiba` `Rio Grande Do Norte`.
